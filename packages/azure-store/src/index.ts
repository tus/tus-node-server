...
import { DataStore, Upload, ERRORS, KvStore, MemoryKvStore, TUS_RESUMABLE } from '@tus/utils'
import { AppendBlobClient, BlobServiceClient, ContainerClient, StorageSharedKeyCredential } from '@azure/storage-blob'
  ...
  */
  ...
  public async create(upload: Upload) {
    ...
    try {
      ...
      upload.storage = { type: 'AzureBlobStore', path: upload.id, bucket: this.containerName }
  ...
  /**
     ...
     */
  public async write(readableStream: http.IncomingMessage | stream.Readable, file_id: string, offset: number): Promise<number> {
