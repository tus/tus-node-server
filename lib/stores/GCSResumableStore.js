...
class GCSResumableStore extends DataStore {
    constructor(options) {
        ...
        else { // running in kubernetes doesn't require a keyfile
            this.gcs = gcloud.storage({
                projectId: options.projectId,
            });
            this.bucket = this._getBucket();

            this.authClient = googleAuth(DEFAULT_CONFIG);
        }


        if (options.redisHost) {
            this.configStore = redis.createClient({
    ...
    write(req, file_id, offset) {
        const self = this;
        return new Promise((resolve, reject) => {
            return self._get(file_id).then((fileObj) => {
                const file = new File(fileObj.id, fileObj.upload_length, fileObj.upload_defer_length, fileObj.upload_metadata, fileObj.uploadUrl);
                if (!file || !file.uploadUrl) {
                    console.log('something went wrong file item cannot be found');
                    self._deleteConfig(file_id);
                    return reject(ERRORS.UNKNOWN_ERROR);
                }

                console.log(`file object ${file_id} returned for write from redis`, file);
                ...
                return self._getRequestStream(reqOpts, (write_stream) => {
                    if (!write_stream) {
                    ...
                    write_stream.on('end', () => {
                        ...
                        offset += new_offset;
                        return self._set(file_id, file).then(() => {
                            if (parseInt(file.upload_length, 10) === offset) {
                                self._deleteConfig(file_id);
                                file.metadata = file.getMetadata();// need for serialization over to host server
    ...
    getOffset(file_id, expectedSize) {
        ...
        return new Promise((resolve, reject) => {
            ...
            return self._get(file_id).then((file) => {
    ...
    _get(file_id) {
        let ret = null;
        if (this.configStore instanceof ConfigStore) {
            const item = this.configStore.get(file_id);
            ...
            ret = new Promise((resolve, reject) => {
                this.configStore.get(file_id, (err, reply) => {
                    console.log(`redis[get]: ${file_id}`, reply);
                    return resolve(reply ? JSON.parse(reply) : null);
                });
                this.configStore.on('error', (err) => {
    ...
    _set(file_id, file) {
        ...
        if (this.configStore instanceof ConfigStore) {
            ...
            ret = new Promise((resolve, reject) => {
                this.configStore.set(file_id, JSON.stringify(file), (err, reply) => {
                    this.configStore.expire(file_id, 604800); // expire in 7 days
                    console.log(`redis[set]: ${file_id}`, file);
                    return resolve();
                });
                this.configStore.on('error', (err) => {
    ...
    _deleteConfig(file_id) {
        ...
        if (this.configStore instanceof ConfigStore) {
            ...
            ret = new Promise((resolve, reject) => {
                this.configStore.del(file_id, (err, reply) => {
                    return resolve();
                });
                this.configStore.on('error', (err) => {
