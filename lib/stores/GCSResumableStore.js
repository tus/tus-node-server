...
class GCSResumableStore extends DataStore {
    ...
    getOffset(file_id, expectedSize) {
        ...
        return new Promise((resolve, reject) => {
            ...
            return self._get(file_id).then((file) => {
                if (!file.uploadUrl) {
                    console.log(`something went wrong the file ${file_id} item cannot be found`);
                    self._deleteConfig(file_id);
                    reject(ERRORS.UNKNOWN_ERROR);
                }

                console.log('file object returned from redis', file);
    ...
    _get(file_id) {
        ...
        if (this.configStore instanceof ConfigStore) {
            ...
            ret = new Promise((resolve, reject) => {
                this.configStore.hgetall(file_id, (err, reply) => {
    ...
    _set(file_id, file) {
        ...
        if (this.configStore instanceof ConfigStore) {
            ...
            ret = new Promise((resolve, reject) => {
                this.configStore.hmset(file_id, file, (err, reply) => {
                    this.configStore.expire(file_id, 30);
                    return resolve();
                });
                this.configStore.on('error', (err) => {
