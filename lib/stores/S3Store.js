...
class S3Store extends DataStore {
   ...
   * @memberof S3Store
   */
    _processUpload(metadata, req, currentPartNumber) {
        const content_length = Number.parseInt(req.headers['content-length'], 10);
      ...
      upload_offset + content_length === upload_length;
        ...
        return new Promise((resolve, reject) => {
            const splitter = new FileStreamSplitter(
                {},
            ...
            stream.pipeline(req, splitter, (pipelineErr) => {
                if (pipelineErr) {
                    ...
                    .then(async() => {
                        ...
                        if (!isFinalRequest) {
                            ...
                            if (stats.size < 5 * 1024 * 1024) {
                                throw new Error('Part too small');
                            }
                        }
                        return undefined;
                    })
            ...
            .then((metadata) => {
                return this._retrieveParts(file_id)
                    ...
                    .catch((err) => {
                        ...
                        return this.client
                            .headObject({ Bucket: this.bucket_name, Key: file_id })
                            .promise()
                            .then(() => {
                                return {
                                    data: {
