...
class S3Store extends DataStore {
     ...
     * @memberof S3Store
     */
    _processUpload(metadata, req, next_part_number) {
        const content_length = Number.parseInt(
            req.headers['content-length'],
            10
        );
        const upload_offset = Number.parseInt(req.headers['upload-offset'], 10);
        const upload_length = Number.parseInt(metadata.file.upload_length, 10);

        const isFinalRequest =
            Number.isFinite(content_length) &&
            Number.isFinite(upload_length) &&
            upload_offset + content_length === upload_length;

        return new Promise((resolve, reject) => {
            let splitter = new FileStreamSplitter(
                {},
                { maxChunkSize: this.part_size, directory: '.' }
            );
            ...
            stream.pipeline(req, splitter, (err) => {
                if (err) {
                    if (pendingChunkFilepath !== null) {
                        fs.rm(pendingChunkFilepath, (err) => {
                            if (err) console.error(err);
                        });
                    }
                }

                promises.push(err ? Promise.reject(err) : Promise.resolve());
                resolve(promises);
            });

            splitter.on('chunkStarted', (filepath) => {
                pendingChunkFilepath = filepath;
            });

            splitter.on('chunkFinished', (filepath) => {
                        ...
                        if (!isFinalRequest) {
                            return new Promise((resolve, reject) => {
                                fs.stat(filepath, (err, stats) => {
                                    ...
                                    // We could modify chunk size in a way that last part is as small as possible.
                                    // Ideally we would not discard any data.
                                    if (stats.size < 5 * 1024 * 1024) {
                                        return reject(
                                            new Error('Part too small')
                                        );
                                    }
                                    resolve();
                                });
    ...
    async _countParts(file_id) {
        const parts = await this._retrieveParts(file_id).then(
            (parts) => parts.length
        );
    ...
    write(req, file_id, offset) {
        return Promise.all([
            ...
            let next_part_number = part_number + 1;
            ...
            return Promise.allSettled(
                ...
                .catch((err) => {
                    if (['RequestTimeout', 'NoSuchUpload'].includes(err.code)) {
                        ...
                        return this.getOffset(file_id, true).then(
