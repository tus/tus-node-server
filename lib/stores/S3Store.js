            ...
            this._getMetadata(file_id)
                .then((metadata) => {
                    return this._retrieveParts(file_id)
                        ...
                        .catch((err) => {
                            if (err.code === 'NoSuchUpload' && with_parts === false) {
                                return this.isUploadFinished(file_id).then((isFinished) => {
                                    if (isFinished) {
                ...
                .then((data) => {
                    if (data.isFinished) {
                        const output = Object.assign({}, this.cache[file_id].file, {
                            size: data.metadata.file.upload_length,
