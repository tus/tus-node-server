    ...
    _processUpload(metadata, req, currentPartNumber) {
        return new Promise((resolve, reject) => {
            const splitter = new FileStreamSplitter(
                {},
                { maxChunkSize: this.part_size, directory: os.tmpdir() }
            );
                    ...
                    .catch((err) => {
                        if (['RequestTimeout', 'NoSuchUpload'].includes(err.code)) {
                            if (err.code === 'RequestTimeout') {
                                log('Request "close" event was emitted, however S3 was expecting more data. Failing gracefully.');
                            }

                            if (err.code === 'NoSuchUpload') {
                                log('Request "close" event was emitted, however S3 was expecting more data. Most likely the upload is already finished/aborted. Failing gracefully.');
                            }
