...
class GetHandler extends BaseHandler {
    ...
    //save path regex in order for using them for url test
    ...
    pathMatch(url){
        for (const [path, pathInfo] of this.paths.entries()) {
            console.log(path, pathInfo);
            return pathInfo.regEx.test(url)
        }
    }
